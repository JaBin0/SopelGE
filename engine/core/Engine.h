...
{
    /*! \brief Indicate if init function of the engine was called and executed correctly */
    bool _isInitialized;

protected:
    /*! \brief Handle to the engine settings */
    Settings& _settings;

    /*! \brief Engine window handle */
    std::unique_ptr<Sopel::IWindow> _window;
...
public: 
    ...
    /*! \brief Method that do actual initialization of the engine 
        This is done to postpone creation of some engine components so,
        settings can be adjusted by the game class beforehand
    */
    void init();

    /*! \brief Start and manage main game loop */
    void run();
